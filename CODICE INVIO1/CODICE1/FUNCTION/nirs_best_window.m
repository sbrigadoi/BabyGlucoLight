function [d_best,t_best,C_thresh,tab_win_after_good,best_five] = nirs_best_window(synchro_result,d,t,rmch,spect_param,J_850,mesh_850,f_disp,disp_single,disp_result)
...
% Indexes and results
idx_best_win = tab_win_after_good.idx(idx_max_mat);
n_best_SNR = SNR_over_snr_th_first(idx_best_win);
n_best_good_ch = tab_win_after_good.n_good(idx_max_mat);
n_best_spars = tab_win_after_good.spars(idx_max_mat);
...
% SD.SrcPos = mesh_recon.source.coord;
% SD.DetPos = mesh_recon.meas.coord;
% 
% figure()
% plotmesh_JAC([mesh3.nodes all_J_best'], mesh3.elements)
% caxis([min_j max_j])
% remCh_best;
% hold on
% plot3(SD.SrcPos(:,1), SD.SrcPos(:,2),SD.SrcPos(:,3),'ro','MarkerSize',20,'LineWidth',15)
% plot3(SD.DetPos(:,1), SD.DetPos(:,2),SD.DetPos(:,3),'bx','MarkerSize',20,'LineWidth',15)
% xlabel('X')
% ylabel('Y')
% zlabel('Z')
% colorbar();
...
%     hold on
%     if remCh_best(iCh) == 1
%         plot3([srcPos(1) detPos(1)],[srcPos(2) detPos(2)],[srcPos(3) detPos(3)],'k')
%     else
%         plot3([srcPos(1) detPos(1)],[srcPos(2) detPos(2)],[srcPos(3) detPos(3)],'c')
%     end
% end
...
if f_disp == 1
    ...
    stem(win_vect,SNR_first_win,'-ok')

    hold on
    plot(idx_over_snr_thr_first,SNR_first_win(idx_over_snr_thr_first),'.g','MarkerSize',15)
    yline(SNR_thr_first,'--r')
    legend('SNR','Win over th','90 Perc th')
    title('SNR for each window')
    xlabel('Window Number')
    ylabel('SNR')
    axis tight
    % print('FIG_27_SNR_green','-djpeg','-r600')
