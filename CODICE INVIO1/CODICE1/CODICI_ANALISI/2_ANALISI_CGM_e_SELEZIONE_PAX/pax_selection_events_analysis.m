function [events_analysis_results] = pax_selection_events_analysis(PD,idx_PD,result_disp,result_plot);
...
%% PLOT EVENTS POST CENSORING AND RESULTS DISPLAY
...
if result_plot == 1
    ...
    title(['CGM DATA PD ',num2str(idx_PD),' POST CENSORING'])
    xlabel('Time [Min]')
    ylabel('[mg/dL]')
    f=get(gca,'Children');
    legend([f(3),f(2),f(1)],'Start Event','End Event','Start Gap')
...
if result_plot == 3
    ...
    xregion(idx_start_event(f_critic_event==0),idx_end_event(f_critic_event==0),"FaceColor",'r')
    end

    if length(idx_start_eu)>0
    xregion(idx_start_eu,idx_end_eu,"FaceColor",'g')
    end
    
    if length(find(f_critic_event==1))>0
        xregion(idx_start_event(f_critic_event==1),idx_end_event(f_critic_event==1),"FaceColor",[0.9290 0.6940 0.1250])
    end

    if f_critic_event(1) == 2
        xregion(idx_start_event(1),idx_end_event(1))
    end
    
    if f_critic_event(end) == 2
        xregion(idx_start_event(end),idx_end_event(end))        
    end
    ...
    f=get(gca,'Children');
    legend([f(3),f(2),f(1)],'Start gap','Start Event','End Event')
    axis([0 length(t) 40 240])
end
