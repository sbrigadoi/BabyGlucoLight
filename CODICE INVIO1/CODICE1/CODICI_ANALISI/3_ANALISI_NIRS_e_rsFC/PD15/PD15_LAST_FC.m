...
% PD 15 LAST EUGLYCEMIA INTERVAL
...
load PD15_NIRS_last_eu_tot.mat
...
disp('Mesh completely loaded')

disp('Jacobian and mesh loaded')
...
% Best 5' time window 
[d_best,t_best,C_thresh,tab_win_after_good,best_five] = nirs_best_window(last_synchro_result,d,t,rmch,spect_param,J_850,mesh_850_33w,f_disp,disp_single,disp_result);
...
% Additional removed channel by visual inspection of the results
removeChSpect([21,21+64,56,56+64]) = 0;
removeChSpect(64) = 0;
removeChSpect(64+64)= 0;
...
title(['RAW NIRS DATA: LAST BEST 5'' WIN, GOOD CH \lambda = ',num2str(SD.Lambda(1)),' nm'])
xlabel('Time [s]')
ylabel('Intensity [A.U.]')
axis tight
...
title(['RAW NIRS DATA: LAST BEST 5'' WIN, GOOD CH \lambda = ',num2str(SD.Lambda(2)),' nm'])
xlabel('Time [s]')
ylabel('Intensity [A.U.]')
axis tight
...
% save PD15_NIRS_best_last.mat t_best d_best d_best_good SD
% disp('Saving complete')

% load('PD15_NIRS_best_last.mat')
...
title(['Optical Density Change Last Best window only good channels, \lambda = ',num2str(SD.Lambda(1)),' nm'])
xlabel('Time [s]')
ylabel('\DeltaOD [A.U.]')
axis tight
...
title(['Optical Density Change Last Best window only good channels, \lambda = ', num2str(SD.Lambda(2)),' nm'])
xlabel('Time [s]')
ylabel('\DeltaOD [A.U.]')
axis tight
...
% save PD_15_LAST_Wavelet.mat dodWavelet dodWavelet_good t_best
% disp('Save wavelet complete')
% load PD_15_LAST_Wavelet.mat

%% START YANG ET AL. METHODS
...
% 
% % DEFINITIVO
% tIncMan = ones(size(dodConv_all,1),1); % Standard
% tMotion = 0.5;
% tMask = 0.75;
% STDEVthresh = 9;
% AMPthresh = 5;
...
% title('DOD after after GWN substitution, Good Channel, First Wavelength')
% axis tight
% 
% subplot(2,1,2)
...
% %                 lh = plot([t(i) t(i)],[-0.3 0.3],'r','LineWidth',0.5);
...
dodFilt =  hmrBandpassFilt(dodGWN,fs,lowerCutOff,higherCutOff);
dodFilt_good = dodFilt(:,removeChSpect==1);

figure()
plot(t_best,dodFilt_good(:,1:(end/2)))
title('DOD after Band Pass filter (0.009 - 0.08) Hz - First Wavelength')
axis tight

figure()
plot(t_best,dodFilt_good(:,(end/2)+1:end))
title('DOD after Band Pass filter (0.009 - 0.08) Hz - Second Wavelength')
axis tight

%% SAVE OF THE DOD AFTER PRE-PROCESSING AND BAND PASS FILTER

% save PD15_NIRS_preprocessed_BP_last.mat dodFilt dodFilt_good t_best SD 
% disp('Saving complete')

% load('PD15_NIRS_preprocessed_BP_last.mat')
...
% load img_recon_PD15_last_eu.mat
...
% save PD_15_LAST_rsFC.mat FC_struct ROI_table
% disp('rsFC saved')

disp('END rsFC ANALYSIS')
