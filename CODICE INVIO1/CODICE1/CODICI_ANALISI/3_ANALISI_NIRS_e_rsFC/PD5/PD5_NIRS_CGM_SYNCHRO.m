% CGM - NIRS SYNCHRONIZATION (SEMI-MANUAL)
...
% ATTENTION: PD5 has one final critic event. Therefore, in order to save
% the patient it is necessary to analyze the NIRS data between the first 
% and the second-last euglycemia interval. From the glycemic analysis,
% these are the results:

% FIRST) 00.00 17 - Apr - 2020 -----> 00.10 17 - Apr - 2020
% LAST ) 22:05 19 - Apr - 2020 -----> 09:00 20 - Apr - 2020

%% SETTING OF FOLDER PATH 

base_path      = pwd;
data_path_cgm  = fullfile(base_path,'DATI','CGM_INTERP');
data_path_nirs = fullfile('E:\DATI\NIRS\PD5\');
...
% Date interval of the first Euglycemia window
date_start_first_eu = date_CGM(PD_results.idx_start_first_eu_w);
date_end_first_eu   = date_CGM(PD_results.idx_end_first_eu_w);

% Since the real last euglycemia interval is after a critical gap, it is
% necessary to analyze the second-last. Between the first and the second
% last there aren't critical gap and the total time of acquisition is above 
% 3 days.
date_start_last_eu  = datetime('19-Apr-2020 22:05:00');
date_end_last_eu    = datetime('20-Apr-2020 09:00:00');
idx_start_last_cgm  = find(date_start_last_eu == date_CGM);
idx_end_last_cgm    = find(date_end_last_eu == date_CGM);
...
tmp = fullfile(data_path_nirs,'*MS PD5 13*.nirs');
...
time_first = split(sep(3),'.');
datetime_format = strcat(sep(5),'-',sep(6),'-',sep(7),{' '},time_first(1),':',time_first(2),':',time_first(3));
date_start_first = datetime(datetime_format);

% In order to get the length of the last acqusition file it is necessary to
% load it
load('MS PD5 13.42.48 Thu 16 Apr 2020 113.nirs','-mat');
n_last = length(t);
...
f_disp = 1; % Display Command Window results
...
[date_start_sincro_last,date_end_sincro_last]   = date_sincro_1_NIRS(date_CGM(idx_start_last_cgm:idx_end_last_cgm),date_start_first,date_end_first,f_disp);
...
if date_start_sincro < date_end_first 
    ...
    dataset_name = 'MS PD5 13.42.48 Thu 16 Apr 2020 %d.nirs';
end
...
% save PD5_synchro_result.mat synchro_result
...
% SPECIFIC FOR PD5
idx_file_Nirs_start_sincro_first = 12;
idx_file_Nirs_end_sincro_first = 12;

idx_file_Nirs_start_sincro_last = 96;
idx_file_Nirs_end_sincro_last = 109;

% First EU
if date_start_first_eu < date_end_first 
    dataset_name_first = 'MS PD5 13.42.48 Thu 16 Apr 2020 %d.nirs';
end

% Last Eu
if date_start_last_eu < date_end_first 
    dataset_name_last = 'MS PD5 13.42.48 Thu 16 Apr 2020 %d.nirs';
end
...
% fold_path = fullfile(base_path,'CODICE/','DEFINITIVO/','NIRS/','PD5/');
% mkdir(fold_path,'PD5_NIRS_DATA_EU/');
% 
% to_save = fullfile(base_path,'CODICE/','DEFINITIVO/','NIRS/','PD5/','PD5_NIRS_DATA_EU/','PD5_NIRS_first_eu_tot.mat');
...
% s = s_tot;
% 
% last_path = fullfile(base_path,'CODICE/','DEFINITIVO/','NIRS/','PD5/','PD5_NIRS_DATA_EU/','PD5_NIRS_last_eu_tot.mat');
