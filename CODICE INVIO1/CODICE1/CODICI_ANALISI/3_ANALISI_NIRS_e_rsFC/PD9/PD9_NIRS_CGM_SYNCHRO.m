...
data_path_nirs = fullfile('E:\DATI\NIRS\PD9\');
...
tmp = fullfile(data_path_nirs,'*PD9 01*.nirs');
info_first_dataset = dir(tmp);
tmp = fullfile(data_path_nirs,'*PD9 bis*.nirs');
info_second_dataset = dir(tmp);
n_dataset_1 = length(info_first_dataset);
n_dataset_2 = length(info_second_dataset);
...
%% FIRST DATASET: EXTRACTION OF ACQUISITION INTERVAL DATE

% Extraction of the first dataset acquisition start date
...
% load it
load('PD9 01.22.01 Sat 09 May 2020 77.nirs','-mat');
n_last = length(t);
...
%% SECOND DATASET: EXTRACTION OF ACQUISITION INTERVAL DATE

% The same procedure was used as the first dataset, paying attention that 
% now the name of the files contain the word 'bis'.

% Extraction of the file name
file_name_second = info_second_dataset.name;
% Separation of the information contained inside the file name and
% conversion in DateTime format
sep = split(string(file_name_second));
time_n2 = split(sep(3),'.');
datetime_format = strcat(sep(5),'-',sep(6),'-',sep(7),{' '},time_n2(1),':',time_n2(2),':',time_n2(3));
date_start_second = datetime(datetime_format);

% In order to get the length of the last acqusition file it is necessary to
% load it
load('PD9 bis 09.47.49 Tue 12 May 2020 29.nirs','-mat');
n_last = length(t);

% Total number of samples acquired in the first dataset and convertion in
% seconds
n2_complete = (n_dataset_2-1)*n_standard;
n2_tot = n2_complete + n_last;
duration_second = seconds(Ts_NIRS*n2_tot); %[s]

% End date of first dataset acquisition
date_end_second = date_start_second + duration_second;
...
disp(' ')
disp('NIRS 2 :')
disp(['Start : ',datestr(date_start_second)])
disp(['End   : ',datestr(date_end_second)])

%% CGM AND NIRS SYNCHRONIZATION

% By the use of the 'date_sincro_2_NIRS' it is extracted the intreval date
% of the CGM-NIRS synchronization in case of 2 NIRS dataset.
f_disp = 1;
[date_start_sincro,date_end_sincro] = date_sincro_2_NIRS(date_CGM,date_start_first,date_end_first,date_start_second,date_end_second,f_disp);
...
f_disp = 1;
[date_start_sincro_first,date_end_sincro_first] = date_sincro_2_NIRS(date_CGM(PD_results.idx_start_first_eu_w:PD_results.idx_end_first_eu_w),date_start_first,date_end_first,date_start_second,date_end_second,f_disp);
[date_start_sincro_last,date_end_sincro_last]  = date_sincro_2_NIRS(date_CGM(PD_results.idx_start_last_eu_w:PD_results.idx_end_last_eu_w),date_start_first,date_end_first,date_start_second,date_end_second,f_disp);
...
if date_start_sincro < date_end_first 
    ...
    dataset_name = 'PD9 01.22.01 Sat 09 May 2020 %d.nirs';
else
    diff_Nirs_sincro = date_start_sincro - date_start_second;
    date_start_Nirs = date_start_second;
    dataset_name = 'PD9 bis 09.47.49 Tue 12 May 2020 %d.nirs';
end
...
if date_start_sincro_first < date_end_first 
    ...
    diff_Nirs_sincro = date_start_sincro_first - date_start_second;
    date_start_Nirs = date_start_second;
    dataset_name = 'PD9 bis 09.47.49 Tue 12 May 2020 %d.nirs';
end
...
if date_start_sincro_last < date_end_first 
    ...
    diff_Nirs_sincro = date_start_sincro_last - date_start_second;
    date_start_Nirs = date_start_second;
    dataset_name = 'PD9 bis 09.47.49 Tue 12 May 2020 %d.nirs';
end
...
% to_save = fullfile(base_path,'CODICE\','DEFINITIVO\','NIRS\','PD9','PD9_synchro_result.mat');
% save (to_save, 'synchro_result')

%% LOAD AND SAVE OF REQUIRED NIRS DATA 

% Da cambiare per altri PAX
                                                                            % SPECIFIC FOR PD9
idx_file_Nirs_start_sincro_first = 0;
idx_file_Nirs_end_sincro_first = 0;

idx_file_Nirs_start_sincro_last = 10;
idx_file_Nirs_end_sincro_last = 10;

% First EU
if date_start_first_eu < date_end_first 
    dataset_name_first = 'PD9 01.22.01 Sat 09 May 2020 %d.nirs';
else
    dataset_name_first = 'PD9 bis 09.47.49 Tue 12 May 2020 %d.nirs';
end

% Last Eu
if date_start_last_eu < date_end_first 
    dataset_name_last = 'PD9 01.22.01 Sat 09 May 2020 %d.nirs';
else
    dataset_name_last = 'PD9 bis 09.47.49 Tue 12 May 2020 %d.nirs';
end
...
% Siccome i due file combaciano carico solo i dati relativi agli indici
% trovati 
...
d_tot   = vertcat(d_tot,d(idx_start_sincro_Nirs_first:idx_end_sincro_Nirs_first,:));
t_tot   = vertcat(t_tot,t(idx_start_sincro_Nirs_first:idx_end_sincro_Nirs_first,:));
aux_tot = vertcat(aux_tot,aux(idx_start_sincro_Nirs_first:idx_end_sincro_Nirs_first,:));
s_tot   = vertcat(s_tot,s(idx_start_sincro_Nirs_first:idx_end_sincro_Nirs_first,:));
...
% 
% 
% fold_path = fullfile(data_path_nirs);
% mkdir(fold_path,'PD9_NIRS_DATA_EU/');
% 
% to_save = fullfile(data_path_nirs,'PD9_NIRS_DATA_EU/','PD9_NIRS_first_eu_tot.mat');
% save (to_save, 'd','t','aux','s','SD')

%% LAST EU INTERVAL (da fare a mano)
...
d_tot   = vertcat(d_tot,d(idx_start_sincro_Nirs_last:idx_end_sincro_Nirs_last,:));
t_tot   = vertcat(t_tot,t(idx_start_sincro_Nirs_last:idx_end_sincro_Nirs_last,:));
aux_tot = vertcat(aux_tot,aux(idx_start_sincro_Nirs_last:idx_end_sincro_Nirs_last,:));
s_tot   = vertcat(s_tot,s(idx_start_sincro_Nirs_last:idx_end_sincro_Nirs_last,:));
...
% s = s_tot;
% 
% last_path = fullfile(data_path_nirs,'PD9_NIRS_DATA_EU','PD9_NIRS_last_eu_tot.mat');
