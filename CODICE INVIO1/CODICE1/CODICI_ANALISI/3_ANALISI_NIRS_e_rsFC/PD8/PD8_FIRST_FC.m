...
% PD 08 FIRST EUGLYCEMIA INTERVAL
...
% low SNR compared to the others. SNR over 1 but under 2
dRange = [5e-4 3];
SNRrange = 2;

% SPECTRAL ANALYSIS PARAMETERS :                                           
% First power threshold was lifted since some good channels where removed
% for power of 0.02 or 0.03.
th_power_1 = 0.05;
th_power_2 = 0.01;
...
load PD8_NIRS_first_eu_tot.mat 
...
% PD08 = 31+0 weeks --> 31 gestation weeks
...
% save PD8_NIRS_best_first.mat t_best d_best d_best_good SD
% disp('Saving complete')

% load('PD8_NIRS_best_first.mat')
...
t_new = Ts*[1:1:size(dodWavelet,1)];
...
%             title(['RAW SIGNAL vs WAVELET CORRECTION - ch: ',num2str(i),])
%         else
%             title(['RAW SIGNAL vs WAVELET CORRECTION - ch: ',num2str(i),])
%         end
%         xlabel('Time [s]')
...
%         set(gcf, 'Position', get(0, 'Screensize'));
%         if i== 76
%             print('FIG_33_CH76_ORvsWavPD8','-djpeg','-r600')
%         end
%         pause
%         close                   
%     end
...
figure()
subplot(2,1,1)
plot(t_new,dodConv_good(:,1:n_ch))
...
subplot(2,1,2)
plot(t_new,dodWavelet_good(:,1:nCh))
...
subplot(2,1,1)
plot(Ts*t_new,dodConv_good(:,n_ch+1:end))
title('RAW NIRS SIGNAL \lambda = 850 nm')
axis tight
ylabel('OD [A.U.]')
xlabel('Time [s]')
ylim([-0.5 1.15])
...
subplot(2,1,2)
plot(Ts*t_new,dodWavelet_good(:,nCh+1:end))
title('AFTER WAVELET CORRECTION NIRS SIGNAL \lambda = 850 nm')
axis tight
ylabel('OD [A.U.]')
xlabel('Time [s]')
ylim([-0.5 1.15])
% print('FIG_32_RAWvsWAV_PD8','-djpeg','-r600')


% save PD_8_FIRST_Wavelet.mat dodWavelet dodWavelet_good t_best
% disp('Save wavelet complete')
% load PD_8_FIRST_Wavelet.mat
...
subplot(2,1,1)
plot(Ts*t_best,dodFilt_good(:,1:(end/2)))
title('OD after Band Pass filter [0.009 0.08] Hz - \lambda = 780 nm')
xlabel('Time [s]')
ylabel('OD [A.U.]')
axis tight
ylim([-0.12 0.08])
...
subplot(2,1,2)
plot(Ts*t_best,dodFilt_good(:,(end/2)+1:end))
title('OD after Band Pass filter [0.009 0.08] Hz - \lambda = 850 nm')
axis tight
ylim([-0.12 0.08])
xlabel('Time [s]')
ylabel('OD [A.U.]')
set(gcf, 'Position', get(0, 'Screensize'));
% print('FIG_34_PB_PD8','-djpeg','-r600')

%% SAVE OF THE DOD AFTER PRE-PROCESSING AND BAND PASS FILTER

% save PD8_NIRS_preprocessed_BP_first.mat dodFilt dodFilt_good t_best SD 
% disp('Saving complete')

% load('PD8_NIRS_preprocessed_BP_first.mat')
...
% r_ROI = 8;   % [mm] NO OVERLAPPING, FUTURE ANALISI ??
...
% save PD_8_FIRST_rsFC.mat FC_struct ROI_table
% disp('rsFC saved')
