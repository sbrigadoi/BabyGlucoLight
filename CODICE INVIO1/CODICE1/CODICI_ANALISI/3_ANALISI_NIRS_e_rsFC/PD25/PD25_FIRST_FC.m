...
% PD 04 FIRST EUGLYCEMIA INTERVAL

% This patient contains two synchronized dataset for the first euglycemia
% interval. It is computed the best 5' window of the first and the second
% dataset and then it will be chose the best one.
...
data_path_nirs = fullfile(base_path,'CODICE','DEFINITIVO/','3_ANALISI_NIRS_e_rsFC/','PD25');
...
% PD09 = 31+3 weeks --> 31 gestation weeks

load J_780_31w.mat
load mesh_780_31w.mat
mesh_780_31w = mesh;

load J_850_31w.mat
load mesh_850_31w.mat
mesh_850_31w = mesh;

disp('Jacobian and mesh completely loaded')


%% FIRST EUGLYCEMIA DATASET
...
%load PD3_synchro_result_1.mat
load('PD25_synchro_result_first_eu.mat')

first_synchro_result = struct();
first_synchro_result.time_start = synchro_result.first_eu.time_start_first_eu_file;
first_synchro_result.idx_start  = synchro_result.first_eu.idx_start_first_eu;
first_synchro_result.time_end   = synchro_result.first_eu.time_end_first_eu_file;
first_synchro_result.idx_end    = synchro_result.first_eu.idx_end_first_eu;

%% FIRST EUGLYCEMIA INTERVAL 
% Load of the NIRS data for first Euglycemia interval
load PD25_NIRS_first_eu_tot.mat
...
spect_param_1 = cell(6,1);
spect_param_1{1} = th_power_1;
spect_param_1{2} = th_power_2;
spect_param_1{3} = f_inf_1;
spect_param_1{4} = f_sup_1;
spect_param_1{5} = f_inf_2;
spect_param_1{6} = f_sup_2;
...
% Best 5' time window 
[d_best_1,t_best_1,C_thresh_1,tab_win_after_good_1,best_five_1] = nirs_best_window(first_synchro_result,d,t,rmch_1,spect_param_1,J_850,mesh_850_31w,f_disp,disp_single,disp_result);
...
figure()
subplot(2,1,1)
plot(t_best_1,d_best_1(:,1:n_ch))
title(['RAW NIRS DATA: FIRST EUGLYCEMIA INTERVAL \lambda = ',num2str(SD.Lambda(1)),' nm'])
xlabel('Time [s]')
ylabel('Intensity [A.U.]')
axis tight

subplot(2,1,2)
plot(t_best_1,d_best_1(:,n_ch+1:end))
title(['RAW NIRS DATA: FIRST EUGLYCEMIA INTERVAL \lambda = ',num2str(SD.Lambda(2)),' nm'])
xlabel('Time [s]')
ylabel('Intensity [A.U.]')
axis tight
...
[removeChSpect_1] = spectral_analysis(d_best_1,Ts,dRange,SNRrange,th_power_1,th_power_2,f_inf_1,f_sup_1,f_inf_2,f_sup_2,disp_single,disp_result);


% 1 GOOD ; 0 REMOVED
d_best_good_1 = d_best_1(:,removeChSpect_1==1);
SD.MeasListAct = removeChSpect_1;
n_ch = size(d_best_good_1,2)/2;

% NIRS SIGNAL DISPLAY
figure()

subplot(2,1,1)
plot(t_best_1,d_best_good_1(:,1:n_ch))
title(['RAW NIRS DATA: FIRST BEST 5'' WIN, GOOD CH \lambda = ',num2str(SD.Lambda(1)),' nm'])
xlabel('Time [s]')
ylabel('Intensity [A.U.]')
axis tight

subplot(2,1,2)
plot(t_best_1,d_best_good_1(:,n_ch+1:end))
title(['RAW NIRS DATA: FIRST BEST 5'' WIN, GOOD CH \lambda = ',num2str(SD.Lambda(2)),' nm'])
xlabel('Time [s]')
ylabel('Intensity [A.U.]')
axis tight
...
for i=1:1:size(d_best_good_1,2)
    meanData = mean(d_best_good_1(:,i));
    stdData  = std(d_best_good_1(:,i),[],1);
    SNRch_1(i)  = meanData./stdData;
end
...
min_snr_1 = min(SNRch_1);
max_snr_1 = max(SNRch_1);
disp(['MIN SNR = ',num2str(min_snr_1)])
disp(['MAX SNR = ',num2str(max_snr_1)])
...
%% END OF FIRST EUGLYCEMIA DATASET

%% SECOND EUGLYCEMIA DATASET
...
% Load of the NIRS data for first Euglycemia interval
load PD4_NIRS_first_eu_tot_2.mat
...
spect_param_2 = cell(6,1);
spect_param_2{1} = th_power_1;
spect_param_2{2} = th_power_2;
spect_param_2{3} = f_inf_1;
spect_param_2{4} = f_sup_1;
spect_param_2{5} = f_inf_2;
spect_param_2{6} = f_sup_2;
...
% Best 5' time window 
[d_best_2,t_best_2,C_thresh_2,tab_win_after_good_2,best_five_2] = nirs_best_window(first_synchro_result,d,t,rmch_2,spect_param_2,J_850,mesh_850_31w,f_disp,disp_single,disp_result);

%% RAW NIRS SIGNAL DISPLAY FOR THE BEST 5' WINDOW 

figure()
subplot(2,1,1)
plot(t_best_2,d_best_2(:,1:n_ch))
title(['RAW NIRS DATA: FIRST EUGLYCEMIA INTERVAL \lambda = ',num2str(SD.Lambda(1)),' nm'])
xlabel('Time [s]')
ylabel('Intensity [A.U.]')
axis tight
...
subplot(2,1,2)
plot(t_best_2,d_best_2(:,n_ch+1:end))
title(['RAW NIRS DATA: FIRST EUGLYCEMIA INTERVAL \lambda = ',num2str(SD.Lambda(2)),' nm'])
xlabel('Time [s]')
ylabel('Intensity [A.U.]')
axis tight
...
[removeChSpect_2] = spectral_analysis(d_best_2,Ts,dRange,SNRrange,th_power_1,th_power_2,f_inf_1,f_sup_1,f_inf_2,f_sup_2,disp_single,disp_result);


% 1 GOOD ; 0 REMOVED
d_best_good_2 = d_best_2(:,removeChSpect_2==1);
SD.MeasListAct = removeChSpect_2;
n_ch = size(d_best_good_2,2)/2;

% NIRS SIGNAL DISPLAY
figure()

subplot(2,1,1)
plot(t_best_2,d_best_good_2(:,1:n_ch))
title(['RAW NIRS DATA: FIRST BEST 5'' WIN, GOOD CH \lambda = ',num2str(SD.Lambda(1)),' nm'])
xlabel('Time [s]')
ylabel('Intensity [A.U.]')
axis tight
...
subplot(2,1,2)
plot(t_best_2,d_best_good_2(:,n_ch+1:end))
title(['RAW NIRS DATA: FIRST BEST 5'' WIN, GOOD CH \lambda = ',num2str(SD.Lambda(2)),' nm'])
xlabel('Time [s]')
ylabel('Intensity [A.U.]')
axis tight
...
for i=1:1:size(d_best_good_2,2)
    meanData = mean(d_best_good_2(:,i));
    stdData  = std(d_best_good_2(:,i),[],1);
    SNRch_2(i)  = meanData./stdData;
end
...
% Display SNR minimum value
min_snr_1 = min(SNRch_2);
max_snr_1 = max(SNRch_2);
disp(['MIN SNR = ',num2str(min_snr_1)])
disp(['MAX SNR = ',num2str(max_snr_1)])
...
%%
% THE BEST 5' TIME WINDOW IS IN THE SECOND DATASET


%% DEFINITIVE BEST 5 MINUTES WINDOW
...
% Best 5' time window 
[d_best,t_best,C_thresh,tab_win_after_good,best_five] = nirs_best_window(first_synchro_result,d,t,rmch,spect_param,J_850,mesh_850_31w,f_disp,disp_single,disp_result);
...
t_best = t_best_1;
d_best = d_best_1;
d_best_good  = d_best_good_1;
SD;
save PD25_NIRS_best_first.mat t_best d_best d_best_good SD
disp('Saving complete')

load('PD25_NIRS_best_first.mat')
...
meanValue_first_good = mean(d_best_good);
dodConv_first_good = -log(abs(d_best_good)./meanValue_first_good);

figure()
subplot(2,1,1)
plot(t_best,dodConv_first_good(:,1:nCh))
title(['Optical Density Change First Best window only good channels, \lambda = ',num2str(SD.Lambda(1)),' nm'])
xlabel('Time [s]')
ylabel('\DeltaOD [A.U.]')
axis tight

subplot(2,1,2)
plot(t_best,dodConv_first_good(:,nCh+1:end))
title(['Optical Density Change First Best window only good channels, \lambda = ', num2str(SD.Lambda(2)),' nm'])
xlabel('Time [s]')
ylabel('\DeltaOD [A.U.]')
axis tight
...
% save PD_4_FIRST_Wavelet.mat dodWavelet dodWavelet_good t_best
% disp('Save wavelet complete')
% load PD_4_FIRST_Wavelet.mat
...
%dodFilt = hmrBandpassFilt(dodConv_all,fs,lowerCutOff,higherCutOff);


removeChSpect = removeChSpect_1;
...
% save PD4_NIRS_preprocessed_BP_first.mat dodFilt dodFilt_good t_best SD 
% disp('Saving complete')

% load('PD4_NIRS_preprocessed_BP_first.mat')
...
% Load age matched meshes
%addpath('E:\DATI\4DNeonatalHeadModel\MODEL\31-weeks')
load('AllMeshes_31weeks.mshs','-mat')

% Load 'vol2gm' matrix, it maps values from volumetric to GM surface 
%addpath 'E:\DATI\4DNeonatalHeadModel\JACOBIAN_AND_MESH\31_WEEKS_JACOBIAN_AND_MESH'
load vol2gm_31w.mat
...
% load img_recon_PD4_first_eu.mat
...
[ROI_table,ROI_active_table,tenTwenty,FC_struct] = rsFC(tenFive,gmSurfaceMesh,vol2gm,all_J,C_thresh,r_ROI,hbo,hbr,f_res);
 ...
 save PD_25_FIRST_rsFC.mat FC_struct ROI_table
 disp('rsFC saved')
